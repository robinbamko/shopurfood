	...
	class DeliveryBoyController1 extends Controller
	{
		...
		public function manage_deliveryboy($id='')
		{
			...
			if($id!='')
			{
				...
				return Redirect::to('manage-deliveryboy-admin1');
			}
			$where = [];
			...
			return view('Admin.DeliveryPerson1.manage_deliveryboy')->with('pagetitle',$page_title)->with('all_details',$get_agent_details)->with('agent_list',$agent_list);
		}
		public function deliveryboy_list_ajax(Request $request)
		{
			if(Session::has('admin_id') == 1)
			{	
				$columns = array(0 => 'deliver_id', 
				1 => 'deliver_id', 
				2 => 'deliver_fname', 
				3=> 'deliver_email',
				4=> 'deliver_phone1',
				...
				{    
					...
					{
						//select `gr_delivery_member`.`deliver_id`, `gr_delivery_member`.`deliver_fname`, `gr_delivery_member`.`deliver_lname`, `gr_delivery_member`.`deliver_email`, `gr_delivery_member`.`deliver_phone1`, `gr_delivery_member`.`deliver_status`,CONCAT(if(gr_delivery_member.deliver_fname is null,'',gr_delivery_member.deliver_fname),' ',if(gr_delivery_member.deliver_lname is null,'',gr_delivery_member.deliver_lname)) as full_name from `gr_delivery_member` where `gr_delivery_member`.`deliver_status` <> '2' order by `deliver_id` desc limit 10 offset 0
				...
				if(!empty($posts))
				{
					...
					$fill_details = (Lang::has(Session::get('admin_lang_file').'.ADMIN_FILL_DETAILS')) ? trans(Session::get('admin_lang_file').'.ADMIN_FILL_DETAILS') : trans($this->ADMIN_LANGUAGE.'.ADMIN_FILL_DETAILS');
					...
					{	
						$tag = '';
						if($post->deliver_newly_register == '1')
						{
							$tag = '<span class="badge bg-danger">New</span>';
						}
						...
						{	
							if($tag != '')
							{
								$statusLink = '<a href="'.url('edit-deliveryboy-admin1').'/'.base64_encode($post->deliver_id).'"><i class="fa fa-ban tooltip-demo" aria-hidden="true" title="'.$fill_details.'" ></i></a>';
							}
							else
							{
						...
						$nestedData['delboyName'] = $post->deliver_fname.' '.$post->deliver_lname.' '.$tag;
						$nestedData['delboyEmail'] = $post->deliver_email;
						$nestedData['delboyPhone'] = $post->deliver_phone1;
						$nestedData['Edit'] = '<a href="'.url('edit-deliveryboy-admin1').'/'.base64_encode($post->deliver_id).'"><i class="fa fa-pencil tooltip-demo" aria-hidden="true" title="'.$click_to_edit.'"></i></a>';
		...
		public function add_deliveryboy()
		{
			...
			$action='save-deliveryboy-admin1';
			$array_name = array();
			...
			return view('Admin.DeliveryPerson1.add_deliveryboy')->with('pagetitle',$page_title)->with('getvendor',$object)->with('id','')->with('action',$action);
		}
		...
		public function save_deliveryboy(Request $request)
		{
			...
			if($res)
			{
				...
				return redirect('manage-deliveryboy-admin1')->with('message',$message);
			}
			else
			{
				...
				return Redirect::to('add-deliveryboy-admin1')->withErrors(['errors'=> $msg])->withInput();
			}
		...
		public function edit_deliveryboy($id)
		{
			...
			$page_title = (Lang::has(Session::get('admin_lang_file').'.ADMIN_DELBOY')) ? trans(Session::get('admin_lang_file').'.ADMIN_DELBOY') : trans($this->ADMIN_LANGUAGE.'.ADMIN_DELBOY');
			$action='update-deliveryboy-admin1';
			return view('Admin.DeliveryPerson1.add_deliveryboy')->with('pagetitle',$page_title)->with('getvendor',$get_merchants_details)->with('id',$id)->with('action',$action);
		}
		...
		public function update_deliveryboy(Request $request)
		{
			...
			if(Input::get('new_register') == '1')
			{
				$profile_det=array_merge($profile_det,['deliver_newly_register' => '0']);
			}
					
			if((Input::get('old_email')!=Input::get('deliver_email')) || (Input::get('old_password') != Input::get('deliver_password')))
			{

	...
	class AdminController extends Controller
	{
		...
		public function admin_dashboard()
		{
			if(Session::has('admin_id') == 1)
			{
				...
				$customer_count 	= Admin::get_count('gr_customer','cus_status','cus_id');
				$merchant_count 	= Admin::get_count('gr_merchant','mer_status','id');
				...
				//print_r($customer_count); exit;
				//DB::connection()->enableQueryLog();count(DISTINCT ord_transaction_id) as total
				$order_count 		= DB::table('gr_order')->select(DB::raw('COUNT(DISTINCT ord_transaction_id) As TotalOrder'))->first()->TotalOrder;
				//$query = DB::getQueryLog();
				//print_r($query);
				//exit;
				$store_active	= Admin::get_activestore_count1('1');
				$store_deactive	= Admin::get_activestore_count1('0');
				$store_count 	= $store_active+$store_deactive;
				
				$restaurant_active	= Admin::get_activerest_count1('1');
				$restaurant_deactive= Admin::get_activerest_count1('0');
				$restaurant_count	= $restaurant_active+$restaurant_deactive;
				$type = 1;
				$product_active = Admin::get_productRitem_count('1',$type);
				$product_deactive = Admin::get_productRitem_count('0',$type);
				//echo $product_active.'/'.$product_deactive;
				//exit;
				$product_count = $product_active+$product_deactive;
				...
				//
				$Customer_normal	= Admin::get_count1('gr_customer','cus_status',['cus_login_type' => '1'],'cus_id');
				$Customer_admin		= Admin::get_count1('gr_customer','cus_status',['cus_login_type' => '2'],'cus_id');
				$Customer_facebook	= Admin::get_count1('gr_customer','cus_status',['cus_login_type' => '3'],'cus_id');
				$Customer_google	= Admin::get_count1('gr_customer','cus_status',['cus_login_type' => '4'],'cus_id');
				$delivered_count	= Admin::get_count2('gr_order',['ord_status' => '7'],'ord_id');
				$merchant_active	= Admin::get_count2('gr_merchant',['mer_status' => '1'],'id');
				$merchant_deactive	= Admin::get_count2('gr_merchant',['mer_status' => '0'],'id');
				
				
				return view('Admin.dashboard')->with(['pagetitle' => $page_title,'customer_count' => $customer_count,'merchant_count' => $merchant_count,'agent_count' => $agent_count,'delivery_count' => $delivery_count,'order_count' => $order_count,'store_count' => $store_count,'restaurant_count' => $restaurant_count,'delivered_count' => $delivered_count,'Customer_normal' => $Customer_normal,'Customer_admin'=>$Customer_admin,'Customer_facebook' => $Customer_facebook,'merchant_active' => $merchant_active,'merchant_deactive' => $merchant_deactive,'store_active' => $store_active,'store_deactive' => $store_deactive,'restaurant_active' => $restaurant_active,'restaurant_deactive' => $restaurant_deactive,'Customer_google' => $Customer_google,'product_active'=>$product_active,'product_deactive'=>$product_deactive,'product_count'=>$product_count,'item_active'=>$item_active,'item_deactive'=>$item_deactive,'item_count'=>$item_count]);
				}else{
		...
		public function check_login(Request $request)
		{
			...
			$email = Input::get('adm_email');
			$pass = Input::get('adm_pass');
		...
		public function profile_submit(Request $request)
		{
            ...
            'adminlastname.required' 		=> $admin_lastname_requried,
		...
		public function ajax_featuredStore_list(Request $request)
		{
			if(Session::has('admin_id') == 1)
			{
				...
				{
					$q = $sql->whereRaw("CONCAT_WS(' ', gr_merchant.mer_fname, gr_merchant.mer_lname, gr_merchant.mer_email) like '%".$merchantName_search."%'");
				}
				if($storeName_search != '')
				{
		...
		public function manage_failed_orders(Request $request)
		{
			...
			//$get_del_failed_orders = Admin::get_failed_orders();

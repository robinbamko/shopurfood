<?php
	...
	class DeliveryBoyController extends Controller
	{
		...
		public function manage_deliveryboy($id='')
		{
			...
			return view('DeliveryManager.DeliveryPerson.manage_deliveryboy')->with('pagetitle',$page_title)->with('all_details',$get_agent_details)->with('agent_list',$agent_list);
		}
		public function deliveryboy_list_ajax(Request $request)
		{
			...
			{    
				...
				if($delboyName_search != '')
				{
					...
					$q = $sql->whereRaw("CONCAT(if(gr_delivery_member.deliver_fname is null,'',gr_delivery_member.deliver_fname),' ',if(gr_delivery_member.deliver_lname is null,'',gr_delivery_member.deliver_lname)) ?", ['%'.$delboyName_search.'%']); 
				}
			...
			if(!empty($posts))
			{
				...
				foreach ($posts as $post)
				{
					...
					{
						$statusLink = '<a href="'.url('deliveryboy_status').'/'.$post->deliver_id.'/0"><i class="fa fa-check tooltip-demo" aria-hidden="true" title="'.$click_to_block.'"></i></a>';
					}
					else
					{
						$statusLink = '<a href="'.url('deliveryboy_status').'/'.$post->deliver_id.'/1"><i class="fa fa-ban tooltip-demo" aria-hidden="true" title="'.$click_to_unblock.'" ></i></a>';
					}
					...
					$nestedData['Edit'] = '<a href="'.url('edit-deliveryboy').'/'.base64_encode($post->deliver_id).'"><i class="fa fa-pencil tooltip-demo" aria-hidden="true" title="'.$click_to_edit.'"></i></a>';
					$nestedData['Status'] = $statusLink;
					$nestedData['delete'] = '<a href= "'.url('deliveryboy_status').'/'.$post->deliver_id.'/2" title="'.$click_to_delete.'" class="tooltip-demo"><i class="fa fa-trash tooltip-demo" aria-hidden="true" ></i></a>';
					$data[] = $nestedData;
		...
		public function add_deliveryboy()
		{
			...
			return view('DeliveryManager.DeliveryPerson.add_deliveryboy')->with('pagetitle',$page_title)->with('getvendor',$object)->with('id','')->with('action',$action)->with('agents_list',$agents_list);
		}
		...
		public function save_deliveryboy(Request $request)
		{
			$this->DELMGR_OUR_LANGUAGE = \Request::get('DELMGR_OUR_LANGUAGE');
			$page_title = (Lang::has(Session::get('DelMgr_lang_file').'.DELMGR_ADD_DELBOY')) ? trans(Session::get('DelMgr_lang_file').'.DELMGR_ADD_DELBOY') : trans($this->DELMGR_OUR_LANGUAGE.'.DELMGR_ADD_DELBOY');
			//$agents_list = DB::table('gr_agent')->select('agent_id',DB::raw("concat(agent_fname, ' - ', agent_email) as full_name"))->where(['agent_status' => '1'])->pluck('full_name','agent_id')->prepend((Lang::has(Session::get('DelMgr_lang_file').'.DELMGR_SELECT')) ? trans(Session::get('DelMgr_lang_file').'.DELMGR_SELECT') : trans($this->DELMGR_OUR_LANGUAGE.'.DELMGR_SELECT'), '0')->toarray();
			...
			'deliver_agent_id.required'    => (Lang::has(Session::get('DelMgr_lang_file').'.DELMGR_SEL_AGENT')) ? trans(Session::get('DelMgr_lang_file').'.DELMGR_SEL_AGENT') : trans($this->DELMGR_OUR_LANGUAGE.'.DELMGR_SEL_AGENT'), 
			'deliver_agent_id.not_in'    => (Lang::has(Session::get('DelMgr_lang_file').'.DELMGR_SEL_AGENT')) ? trans(Session::get('DelMgr_lang_file').'.DELMGR_SEL_AGENT') : trans($this->DELMGR_OUR_LANGUAGE.'.DELMGR_SEL_AGENT'), 
			...
			'deliver_fname'=> mysql_escape_special_chars(Input::get('deliver_fname')),
			'deliver_lname'=> mysql_escape_special_chars(Input::get('deliver_lname')),
			'deliver_email'=> mysql_escape_special_chars(Input::get('deliver_email')),
			'deliver_password'=>md5($passwordIs),
			'deliver_decrypt_password'=>$passwordIs,
			'deliver_phone1'=> mysql_escape_special_chars(Input::get('deliver_phone1')),
			'deliver_phone2'=> mysql_escape_special_chars(Input::get('deliver_phone2')),
			'deliver_state'=> Input::get('deliver_state'),
			'deliver_location'=> mysql_escape_special_chars(Input::get('deliver_location')),
			...
			'deliver_status'=>'1',
			'deliver_currency_code'=> mysql_escape_special_chars(Input::get('deliver_currency_code')),
			'deliver_base_fare'=> mysql_escape_special_chars(Input::get('deliver_base_fare')),
			'deliver_fare_type'=> mysql_escape_special_chars(Input::get('deliver_fare_type')),
			'deliver_perkm_charge'=> mysql_escape_special_chars(Input::get('deliver_perkm_charge')),
			'deliver_permin_charge'=> mysql_escape_special_chars(Input::get('deliver_permin_charge')),
			'deliver_vehicle_details'=> mysql_escape_special_chars(Input::get('deliver_vehicle_details')),
			'deliver_order_limit'=> mysql_escape_special_chars(Input::get('deliver_order_limit')),
			...
			if($res)
			{
				...
				return redirect('manage-deliveryboy')->with('message',$message);
			}
			else
			{
				...
				return Redirect::to('add-deliveryboy')->withErrors(['errors'=> $msg])->withInput();
			}
		...
		public function edit_deliveryboy($id)
		{
			...
			//$agents_list = DB::table('gr_agent')->select('agent_id',DB::raw("concat(agent_fname, ' - ', agent_email) as full_name"))->where(['agent_status' => '1'])->pluck('full_name','agent_id')->prepend((Lang::has(Session::get('DelMgr_lang_file').'.DELMGR_SELECT')) ? trans(Session::get('DelMgr_lang_file').'.DELMGR_SELECT') : trans($this->DELMGR_OUR_LANGUAGE.'.DELMGR_SELECT'), '0')->toarray();
			$agents_list = DB::table('gr_agent')->select('agent_id',DB::raw("concat(if(agent_fname is null,'',agent_fname), ' - ', if(agent_email is null,'',agent_email)) as full_name"))->where(['agent_status' => '1'])->pluck('full_name','agent_id')->prepend((Lang::has(Session::get('DelMgr_lang_file').'.DELMGR_SELECT')) ? trans(Session::get('DelMgr_lang_file').'.DELMGR_SELECT') : trans($this->DELMGR_OUR_LANGUAGE.'.DELMGR_SELECT'), '0')->toarray();
			$action='update-deliveryboy';
			return view('DeliveryManager.DeliveryPerson.add_deliveryboy')->with('pagetitle',$page_title)->with('getvendor',$get_merchants_details)->with('id',$id)->with('action',$action)->with('agents_list',$agents_list);
		}
		...
		{	
			...
			{	
				...
				Session::flash('message',$msg);
				return Redirect::to('manage-deliveryboy')->withErrors(['success'=>$msg])->withInput();
			}
			if($status == 2) //Delete
			{	

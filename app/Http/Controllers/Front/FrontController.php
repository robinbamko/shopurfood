<?php 
	...
	class FrontController extends Controller
	{
		...
		public function all_categories(Request $request){
			$id = mysql_escape_special_chars($request->id); //used for specific category
			$top_disc_filter = mysql_escape_special_chars($request->top_disc_filter);
			$del_time_filter = mysql_escape_special_chars($request->del_time_filter);
			$rate_filter = mysql_escape_special_chars($request->rate_filter);
			$cuisines = mysql_escape_special_chars($request->cuisines);
			$text = mysql_escape_special_chars($request->text);
			//echo $id.'/'.$top_disc_filter.'/'.$del_time_filter.'/'.$rate_filter.'/'.$text.'<pre>';  print_r($cuisines); exit;
			...
			$search_loaction = Session::get('search_location');
			$latitude = (double)Session::get('search_latitude');
			$langitude = (double)Session::get('search_longitude');
			$restaurant_lists = Home::get_category_shops_byCategory_pofi('1',24,$latitude,$langitude, $top_disc_filter, $del_time_filter, $rate_filter, $text, $cuisines); 
		...
		public function session_location(Request $request)
		{
			...
			if(Session::has('search_location') == 1)
			{
				...
				else
				{
					...
					{
						$request->latitude = (double)$request->latitude;
						$request->longitude = (double)$request->longitude;
						echo '1';
			...
			else
			{
				...
				$request->latitude = (double)$request->latitude;
				$request->longitude = (double)$request->longitude;
				$request->location = mysql_escape_special_chars($request->location);
		...
		public function user_change_password_submit(Request $request)
		{
			...
			$current_password = mysql_escape_special_chars(Input::get('currentpassword'));
			$new_password = mysql_escape_special_chars(Input::get('newpassword'));
			$confirmpassword = mysql_escape_special_chars(Input::get('confirmpassword'));
		...
		{
			$customer_id = (int)Session::get('customer_id');
			$order_number = mysql_escape_special_chars(Input::get('order_number'));
		...
		public function OrderDetailToCancel($id)
		{
			...
			$ord_transaction_id = (int)$ord_transaction_id;
			...
			$Invoice_Order = DB::table('gr_order')->select('gr_order.ord_id',
													...
													DB::Raw('(SELECT(Case When (gr_customer.cus_paymaya_status="Unpublish" AND gr_customer.cus_netbank_status="Unpublish") Then 0 Else 1 End)) AS payment_status')
													)
													->join('gr_store','gr_order.ord_rest_id', '=', 'gr_store.id')
													->join('gr_customer','gr_order.ord_cus_id', '=', 'gr_customer.cus_id')
													->join('gr_product','gr_order.ord_pro_id','=','gr_product.pro_id')
		...
		{	
			...
			if($_POST) { 
				...
				/* /don't force to fill paypal or netbanking details  
				...
				if(Session::has('redirect_to') == 1)
				{	
					$url = Session::get('redirect_to');
					Session::forget('redirect_to');
					//echo Session::has('redirect_to'); exit;
					return redirect($url);
				}
				else
				{
		...
		{
			$refer_mail = mysql_escape_special_chars($request->mail);
		...
		public function set_redirect_url(Request $request)
		{
			Session::put('redirect_to',$request->redirect_url);
			echo Session::get('redirect_to');
		}
	}			

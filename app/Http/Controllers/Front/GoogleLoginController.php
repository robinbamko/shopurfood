<?php 
	...
	class GoogleLoginController extends Controller
	{
		...
		public function handleGoogleCallback()
		{
			...
			try {
				...
				if(empty($check_already_exsist) === false)
				{
					...
					if($check_already_exsist->cus_status=='0')
					{
					...
					$update = ['cus_google_id' => $create['google_id']];
				...
				else
				{
					...
					'cus_google_id' => $create['google_id'], 
					'cus_status' => '1', 
					'cus_login_type'=>'4',
		...
		public function Google_Logout(Request $request)
		{
			Session::flush();
			Auth::logout();
			$msg = (Lang::has(Session::get('front_lang_file').'.FRONT_LOGOUT_SUCCESS')) ? trans(Session::get('front_lang_file').'.FRONT_LOGOUT_SUCCESS') : trans($this->FRONT_LANGUAGE.'.FRONT_LOGOUT_SUCCESS');

<?php 
	...
	class PaymayaController extends Controller
	{
		...
		public function paymaya_checkout1(Request $request)
		{	
			PayMayaSDK::getInstance()->initCheckout(Config::get('env.PAYMAYA_PUBLIC_KEY'),Config::get('env.PAYMAYA_SECRET_KEY'),(\App::environment('production') ? 'SANDBOX' : 'SANDBOX'));
		...
		public function paymaya_checkout(Request $request)
		{
			...
			PayMayaSDK::getInstance()->initCheckout(Config::get('env.PAYMAYA_PUBLIC_KEY'),Config::get('env.PAYMAYA_SECRET_KEY'),Config::get('env.PAYMAYA_MODE'));
			...
			else
			{
				...
				{	
					...
					{	
						foreach($value as $pdtDetail)
						{
							...
							/*-------- START ITEM DETAILS ----- */
							// Item
							$converted_cart_tax = convertCurrency($pdtDetail->co_curcode, 'PHP', $cart_tax);
							$converted_sub_total = convertCurrency($pdtDetail->co_curcode, 'PHP', $sub_total);
							$converted_cart_unit_amt = convertCurrency($pdtDetail->co_curcode, 'PHP',$pdtDetail->cart_unit_amt);
							$itemAmountDetails = new ItemAmountDetails();
							$itemAmountDetails->tax = $converted_cart_tax;
							$itemAmountDetails->subtotal = $converted_sub_total;
							
							$itemAmount = new ItemAmount();
							$itemAmount->currency = "PHP";
							$itemAmount->value = $converted_cart_unit_amt;//$itemAmountDetails->subtotal;
							$itemAmount->details = $itemAmountDetails;
							
							$itemTotalAmount = new ItemAmount();
							$itemTotalAmount->currency = "PHP";
							$itemTotalAmount->value = ($converted_sub_total-$converted_cart_tax);
							$itemTotalAmount->details = $itemAmountDetails;
				...
				$converted_grand_tax = convertCurrency($pdtDetail->co_curcode, 'PHP', $grand_tax);
				$converted_shippingFee = convertCurrency($pdtDetail->co_curcode, 'PHP', $shippingFee);
				$converted_grand_sub_total = convertCurrency($pdtDetail->co_curcode, 'PHP', $grand_sub_total);
				$converted_wallet_amt = convertCurrency_withoutRound($pdtDetail->co_curcode, 'PHP', $wallet_amt);
				$converted_total_amt_to_pay = convertCurrency($pdtDetail->co_curcode, 'PHP', $total_amt_to_pay);
				$itemAmountDetails = new ItemAmountDetails();
				$itemAmountDetails->tax = $converted_grand_tax;
				$itemAmountDetails->shippingFee = $converted_shippingFee;
				$itemAmountDetails->subtotal = $converted_grand_sub_total;
				$itemAmountDetails->discount = $converted_wallet_amt;
				
				$itemAmount = new ItemAmount();
				$itemAmount->currency = "PHP";
				$itemAmount->value = $converted_total_amt_to_pay;
				$itemAmount->details = $itemAmountDetails;
		...
		{	
			...
			*/
			...
			if($checkout['paymentStatus'] == "PAYMENT_SUCCESS")
			{
				...
				$converted_discount = convertCurrency('PHP',Session::get('default_currency_code'), $checkout['totalAmount']['details']['discount']);
				$updat = updatevalues('gr_customer',['used_wallet' => DB::raw('"used_wallet"+'.$converted_discount)],['cus_id' => $cus_id]);
				...
				if(count($cartSaveDet) > 0 )
				{
					foreach($cartSaveDet as $cartDet)
					{
						...
						$shippingFee = ($selfPickupStatus==0) ? Config::get('delivery_fee') : '0.00';
						...
						'ord_paymaya_pmtId'		=> $requestReferenceNumber,
						'ord_paymaya_receiptnum'	=> $requestReferenceNumber,
						'ord_paymaya_paid_time'	=> $requestReferenceNumber,
						'ord_paymaya_last4'		=> $requestReferenceNumber,
						'ord_paymaya_carttype'	=> $requestReferenceNumber,
						'ord_paymaya_maskedcart'	=> $requestReferenceNumber,
		...
		public function checkout_failure(Request $request)
		{
			...
			return Redirect::to('/')->withErrors(['message' => 'Payment Failure <br> Error Code: '.$checkout['paymentStatus']]);
			/*$itemCheckout = new Checkout();

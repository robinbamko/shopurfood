<?php
	...
	class Item extends Model
	{
		...
		/** get all details **/
		public static function get_all_detail($type)
		{
			
			//DB::connection()->enableQueryLog();
			return DB::table('gr_product')->select('gr_product.pro_id','gr_product.pro_store_id','gr_product.pro_status','gr_store.st_store_name','gr_product.pro_item_code','gr_product.pro_item_name','gr_product.pro_original_price','gr_product.pro_discount_price','gr_product.pro_images','gr_product.added_by','gr_merchant.mer_email',DB::Raw("IF(('gr_product.pro_quantity' > 'gr_product.pro_no_of_purchase'),'Avail','Sold') as availablity"))
		...
		public static function get_merchants_all_details($type,$pdt_status)
		{
			$store_id = DB::table('gr_store')->select('id')->where('st_mer_id','=',Session::get('merchantid'))->first();
			$storeidIs = $store_id->id;
			//DB::connection()->enableQueryLog();
			$sql =  DB::table('gr_product')->select('gr_product.pro_id','gr_product.pro_store_id','gr_product.pro_status','gr_store.st_store_name','gr_product.pro_item_code','gr_product.pro_item_code','gr_product.pro_item_name','gr_product.pro_original_price','gr_product.pro_discount_price','gr_product.pro_images','gr_product.added_by','gr_merchant.mer_email',DB::Raw(" IF(((gr_product.pro_quantity- gr_product.pro_no_of_purchase) > 0),'Avail','Sold') as availablity"))
			->Join('gr_store','gr_store.id','=','gr_product.pro_store_id')
			->Join('gr_merchant','gr_store.st_mer_id','=','gr_merchant.id')
			->where('gr_merchant.mer_status','<>','2')
			->where('gr_store.st_status','<>','2')
		...
		{
			//DB::connection()->enableQueryLog();
			return DB::table('gr_store')->select('gr_store.st_store_name','gr_merchant.mer_email','gr_store.id','gr_merchant.id as mer_id')
			->Join('gr_merchant','gr_store.st_mer_id','=','gr_merchant.id')
			->where('gr_store.st_status','<>','2')
			->where('gr_merchant.mer_status','<>','2')

								...
								<table class="table table-striped table-bordered table-hover" id="dataTables-example" style="text-align:center">
									<thead>
										<tr>
											...
											@if((isset($CMS) && is_array($CMS)) && in_array('2', $CMS) || $allPrev == '1')
											<th>
											...
											@if((isset($CMS) && is_array($CMS)) && in_array('3', $CMS) || $allPrev == '1')
											<th>
										...
										<tr>
											...
											<td>{{$i}}</td>
											<td>{{ ucfirst($details->page_title_en) }}</td>
											@if((isset($CMS) && is_array($CMS)) && in_array('2', $CMS) || $allPrev == '1')
										    <td>
												<a href="{!! url('edit-cms').'/'.base64_encode($details->id) !!}"><i class="fa fa-pencil tooltip-demo" aria-hidden="true" title="Edit"></i></a>
											</td>
											@endif
											<td>
												{{-- Don't make any changes in help and about us cms --}}
												@if(in_array($details->slug,['help','about-us','terms','privacy-policy']))
												{{"-"}}
												@else
													@if($details->page_status == 1)  {{--0-block, 1- active --}}
													<a href="{!! url('cms_status').'/'.$details->id.'/0' !!}"><i class="fa fa-check tooltip-demo" aria-hidden="true" title="Click to Block"></i></a>
													@else
													<a href="{!! url('cms_status').'/'.$details->id.'/1' !!}"><i class="fa fa-ban tooltip-demo" aria-hidden="true" title="Click to Unblock" onClick="change_status($details->id)"></i></a>
													@endif
												@endif
											</td>
											@if((isset($CMS) && is_array($CMS)) && in_array('3', $CMS) || $allPrev == '1')
											<td>
												...
												{{"-"}}
												@else
												<a href= "{!! url('cms_status').'/'.$details->id.'/2' !!}" title="delete" class="tooltip-demo">

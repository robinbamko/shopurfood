...
@extends('Front.layouts.default')
			...
			<div class="container">
				<div class="row">
					<div class="col-md-12 all-heading">
						<div class="row">
							<div class="col-md-10">
								<h4>@lang(Session::get('front_lang_file').'.FRONT_ALL_RESTS')</h4>

							</div>
							<div class="col-md-2">
								@if(count($all_restaurants) > 5)
								<a href="{{url('all-categories')}}"><h5>@lang(Session::get('front_lang_file').'.FRONT_SEE_ALL')<span>&gt;</span></h5> </a>
								@endif
							</div>
						</div>
					</div>
					<section class="regular2 slider your-carousel">
						@if(count($all_restaurants) > 0)
						@foreach($all_restaurants as $restaurant)
						 @php $filename = public_path('images/restaurant/').$restaurant->st_logo; @endphp
						<div>
							<a href="{{url('restaurant').'/'.$restaurant->store_slug}}" title="{{ucfirst($restaurant->st_name)}}">
							@if($restaurant->st_logo != '' && file_exists($filename))
							<img src="{{url('')}}/public/images/restaurant/{{$restaurant->st_logo}}">
							@else
	...
	{{-- initialize max page count --}}
	 @php $page_count_max = 1 ; @endphp
	  ...
	  <div class="ajax-loading"><!-- @lang(Session::get('front_lang_file').'.LOAD') --> <img src="{{url('')}}/public/front/images/load.gif"></div>
	  <div class="section7">
		<div class="container">
			@if(count($all_restaurants) > 0)
			<div class="row">
				<div class="col-md-12">
					<a href="{{url('all-categories')}}">@lang(Session::get('front_lang_file').'.FRONT_ALL_CATEGORIES')<span>&gt;&gt;</span></a>
				</div>
			</div>
			@endif
		</div>

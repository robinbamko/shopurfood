					...
					<table style="font-family:sans-serif; font-size:14px; width:100%; padding:30px 20px;"  align="center">
						...
						<tr>
							<td colspan="2" style="padding-bottom:5px; font-family:sans-serif; font-size:16px;font-weight: 600;">
								@if($status == 0)
									@php echo __(Session::get('admin_lang_file').'.ADMIN_CH_DM_STATUS',['status' => 'blocked']); @endphp
								@elseif($status == 1)
									@php echo __(Session::get('admin_lang_file').'.ADMIN_CH_DM_STATUS',['status' => 'unblocked']); @endphp 
								@elseif($status == 2)
									@php echo __(Session::get('admin_lang_file').'.ADMIN_CH_DM_STATUS',['status' => 'deleted']); @endphp
								@endif
